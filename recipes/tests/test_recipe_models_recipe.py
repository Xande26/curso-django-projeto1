...
class RecipeModelTest(RecipeTestBase):
    ...
    def make_recipe_no_defalt(self):
        recipe = Recipe(
    ...
    def test_recipe_fields_max_lenght(self, field, max_lenght):
        setattr(self.recipe, field, 'U' * (max_lenght + 1))
        with self.assertRaises(ValidationError):
            self.recipe.full_clean()

    def test_recipe_preparation_steps_is_html_is_false_by_default(self):
        recipe = self.make_recipe_no_defalt()
        self.assertFalse(
            recipe.preparation_steps_is_html,
            msg='Recipe preparation_time_unit is not false')
    ...
    def test_recipe_string_representation(self):
        ...
        self.assertEqual(
            ...
            msg=f'recipe string representation must be ' 
                f' ""{needed}" but "{str(self.recipe)}" was receives.')